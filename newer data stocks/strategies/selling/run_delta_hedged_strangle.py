#!/usr/bin/env python3
"""
DELTA-HEDGED SHORT STRANGLE - Professional Income Strategy

Strategy:
1. Entry: 9:20 AM - Sell ATM Call + ATM Put (collect premium from both)
2. Monitor: Track delta (price change per underlying movement) continuously
3. Profit Taking: When one leg's delta is significantly higher (winner), exit that leg
4. Re-hedging: If loser approaches breakeven/loss and mean doesn't revert, hedge again
5. Exit: Close all positions at 3:20 PM EOD

Why it works:
- Collect theta decay from both sides
- Take profits on winning side when delta diverges
- Re-hedge to limit losses on losing side
- Delta monitoring ensures we capture directional moves efficiently

Key Metrics:
- Delta ratio threshold: 2.0× (one side moves 2× faster than other)
- Re-hedge trigger: When P&L on remaining leg < -50% of premium collected
- Max positions: 2 legs active at any time
"""
...
def calculate_delta(price_change: float, spot_change: float) -> float:
    """Calculate option delta (price change / underlying change)"""
...
def find_atm_strikes(spot_price: float, strikes_arr: np.ndarray, opt_types_arr: np.ndarray, 
    ...
    # Find ATM CE (strike at or above spot)
    for i in range(start_idx, end_idx):
        if opt_types_arr[i] == 0 and abs(strikes_arr[i] - rounded_spot) < 1.0:
            ce_idx = i
            break
    
    # Find ATM PE (strike at or below spot)
    for i in range(start_idx, end_idx):
...
@njit
def strategy_delta_hedged_strangle(
    ...
    """
    Delta-Hedged Short Strangle Strategy
    
    Sell ATM straddle, manage legs based on delta, re-hedge when needed
    """
    n = len(prices)
    max_trades = 100  # Conservative for selling strategies
    ...
    while i < n:
        ...
        # === ENTRY LOGIC ===
        if not in_strangle and current_time >= ENTRY_TIME and current_time < ENTRY_TIME + 600:  # 10-min window
            # Find ATM strikes
        ...
        # === POSITION MANAGEMENT ===
            ...
            # === PROFIT TAKING LOGIC ===
            # If one leg's delta is 2× the other, it's the winner - exit it
            profit_take_threshold = 2.0
            
            if ce_active and pe_active:
                # Both legs active - check delta divergence
                if ce_delta > 0 and pe_delta > 0:
                    delta_ratio = ce_delta / pe_delta if pe_delta > 0 else 0.0
                    
                    # CE is winning (moving faster)
                    if delta_ratio > profit_take_threshold and ce_pnl > 0:
                        ce_active = False
                    ...
                    # PE is winning
                    elif delta_ratio < (1.0 / profit_take_threshold) and pe_pnl > 0:
                        pe_active = False
            ...
            # === RE-HEDGE LOGIC ===
            # If one leg closed and other is losing >50% of premium, add hedge
            rehedge_trigger = -0.5  # Lose 50% of collected premium
            
            if ce_active and not pe_active:
                # Only CE active, check if losing
                if ce_pnl < (ce_entry_price * rehedge_trigger):
                    # Add PE hedge
                    pe_idx_new = -1
                    for j in range(block_start, block_end):
                ...
                # Only PE active, check if losing
                if pe_pnl < (pe_entry_price * rehedge_trigger):
                    # Add CE hedge
                    ce_idx_new = -1
                    for j in range(block_start, block_end):
            ...
            if current_time >= EOD_EXIT:
                ...
                if trade_count < max_trades:
                    final_ce_pnl = ce_entry_price - ce_exit_price - transaction_cost
                    final_pe_pnl = pe_entry_price - pe_exit_price - transaction_cost
                    total_pnl = final_ce_pnl + final_pe_pnl
...
def run_delta_strangle(data_dir: Path, underlying: str) -> List[StrangleTrade]:
    """Run delta-hedged short strangle strategy"""
    all_trades = []
    
    TRANSACTION_COST = 5.0  # ₹5 per leg
    ...
    for date_dir in date_dirs:
        ...
        try:
            ...
            results = strategy_delta_hedged_strangle(
            ...
            if len(results[0]) > 0:
                ...
                exit_reason_map = {0: 'profit_take', 1: 'stop', 2: 'eod'}
...
def main():
    ...
    print("="*80)
    print("DELTA-HEDGED SHORT STRANGLE - Professional Income Strategy")
    print("="*80)
    print("Strategy Logic:")
    print("  1. Entry: 9:20 AM - Sell ATM Call + Put")
    print("  2. Monitor: Track delta continuously")
    print("  3. Profit: Exit winner when delta ratio > 2.0×")
    print("  4. Re-hedge: Add opposite leg if loser down >50%")
    print("  5. Exit: Close all at 3:20 PM")
    print("")
    ...
    for underlying in ['BANKNIFTY', 'NIFTY']:
        ...
        trades = run_delta_strangle(data_dir, underlying)
        ...
        if trades:
            ...
            output_file = results_dir / f"{underlying}_delta_strangle_trades.csv"
    ...
    if all_results:
        summary_file = results_dir / "delta_strangle_summary.csv"
