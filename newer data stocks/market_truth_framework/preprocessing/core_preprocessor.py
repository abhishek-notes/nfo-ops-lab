...
Outputs:
- features_{date}.parquet: Per-second state vectors (RV, spreads, depths, OBI, etc.)
- bursts_{date}.parquet: All burst events with option responses
- options_atm_{date}.parquet: ATM CE/PE detailed data
...
def get_burst_params(dte: int, underlying: str):
    """Get burst detection parameters based on DTE"""
    # Simplified for Numba - use hardcoded params
    if underlying == 'NIFTY':
        if dte == 0:
            return 8.0, 2.0, 1.6, 1.2, 2.0  # B_points, B_step, k1, k2, end_points
        elif dte <= 2:
...
def preprocess_day(date: str, underlying: str, data_dir: Path, output_dir: Path):
    ...
    # Get unique timestamps (1-second grid)
    print("Creating per-second aggregation...")
    timestamps = df['timestamp'].unique().sort()
    
    print(f"  {len(timestamps):,} unique timestamps")
    
    # Build ATM time series
    ...
    for ts in timestamps:
        ...
        # Extract first row (should be only one per strike/type)
        ce_row = ce_data.row(0, named=True)
        pe_row = pe_data.row(0, named=True)
        
        # Compute microstructure for both
    ...
    print(f"  Created {len(features_df):,} per-second rows")
    ...
    # Features
    features_file = output_dir / 'features' / f'features_{date}.parquet'
    ...
    if burst_events:
        ...
        bursts_file = output_dir / 'bursts' / f'bursts_{date}.parquet'
        bursts_df.write_parquet(bursts_file)
        print(f"  ✓ Saved bursts: {bursts_file}")
    
    print(f"\n✓ Processing complete for {date}")
...
def main():
    ...
    preprocess_day(args.date, args.underlying, data_dir, output_dir)
