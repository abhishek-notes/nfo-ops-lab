...
def run(symbol,start,end,short_secs=3,price_thresh=2.0,hold_secs=10):
    ...
    while d<=d1:
        ...
        for i,(u,dn) in enumerate(zip(up.to_list(), down.to_list())):
            if not (u or dn):
                continue
            entry_ts=s_ts[i]; entry_spot=float(s_close[i])
            # exit in hold_secs or last row
            ...
            if u:
                pnl=0.5*(exit_spot-entry_spot)
                side='buy_up'
            else:
                pnl=0.5*(entry_spot-exit_spot)
                side='sell_down'
    ...
    if out:
        ...
        outp=OUT_DIR/f"momentum_sanity_{symbol}_{start}_{end}.parquet"
...
if __name__=='__main__':
    ...
    a=ap.parse_args()
    run(a.symbol,a.start,a.end,a.short_secs,a.price_thresh,a.hold_secs)
