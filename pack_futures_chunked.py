#!/usr/bin/env python3
# Memory-efficient futures packing - processes large files in chunks
...
def process_chunk(df: pl.DataFrame, sym: str, cal: pl.DataFrame) -> pl.DataFrame | None:
    ...
    if cal_s.is_empty(): return None
    df = df.join_asof(cal_s, left_on="trade_date", right_on="expiry", strategy="forward")
    return df.drop_nulls(["expiry"])

def process_file_chunked(path: str, cal: pl.DataFrame) -> dict[tuple[str, str], list[pl.DataFrame]]:
    sym = infer_symbol(path)
    if not sym: 
        print(f"skip (symbol?): {path}")
        return {}
    
    print(f"Processing {path} in chunks...")
    ...
    for i in range(0, num_rows, CHUNK_SIZE):
        ...
        # Read chunk using pyarrow
        table = parquet_file.read_row_group(i // parquet_file.metadata.row_group(0).num_rows)
        df = pl.from_arrow(table)
        
        # Process chunk
        df_processed = process_chunk(df, sym, cal)
        if df_processed is None or df_processed.height == 0:
            continue
            
        # Group by expiry
        for exp in df_processed.select("expiry").unique().to_series():
            g = df_processed.filter(pl.col("expiry")==exp)
...
def main():
    ...
    all_buckets: dict[tuple[str, str], list[pl.DataFrame]] = {}
    
    for i, p in enumerate(files, 1):
        print(f"\nFile {i}/{len(files)}: {os.path.basename(p)}")
        try:
            file_buckets = process_file_chunked(p, cal)
            # Merge buckets
            for key, chunks in file_buckets.items():
                all_buckets.setdefault(key, []).extend(chunks)
        except Exception as e:
            print(f"Error processing {p}: {e}")
            continue

    print(f"\nWriting {len(all_buckets)} futures partitions...")
    for (sym, exp), chunks in all_buckets.items():
